...
import { hashPassword, createAccessToken, verifyToken } from "../src/core/security.js";
import { User } from "../src/models/user.js";
import { initialUser } from "./api-test-utils.js";
...
describe("when there is initially an user seeded", () => {
  ...
  describe("logging in", () => {
    test("succeeds with correct credentials and returning user details and the token", async () => {
      ...
      expect(res.body.username).toBe(initialUser.username);
      expect(res.body.name).toBe(initialUser.name);
    });
    ...
    test("succeeds with the valid issued token that contains the correct sub", async () => {
      ...
      expect(decoded.sub).toBe(initialUser.username);
    });
  ...
  describe("access token", () => {
    ...
    test("fails verification if the token is invalid", () => {
      ...
      expect(() => verifyToken(brokenToken)).toThrow(jwt.JsonWebTokenError);
    });

    test("fails verification if the token is expired", () => {
      // Create a token that expires in 5 minutes
      const token = createAccessToken(payload, 5);
      // Should work fine immediately
      expect(verifyToken(token)).toBe(payload.sub);
